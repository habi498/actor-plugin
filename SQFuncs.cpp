...
void leftshift(unsigned char* p, unsigned char n)
{
...
public:
	...
	bool pending4 = false;
	unsigned char weapon = 0x11;
	float shootx = 0, shooty = 0, shootz = 0;
...
{
	//len is lenght of packet. full length.
	int len2 = len, m1,m2;
...
void encdAngle(float angl, char* a, char* b)
{
	int val = floor(angl * 4096 * 4 / PI + 32767);
	encpld(val, a, b);
}
void Actor::Send94()
{
	...
	encpld(47 * 8, &pkt[5], &pkt[6]);//PAYLOAD
	...
	angle = atan2(-(shootx - px), shooty - py);
	encodeAngle(&pkt[32], &pkt[33]);
	pkt[34] = weapon;
	pkt[35] = 255;//ammo field 1
	pkt[36] = 255;//ammo field 2
	pkt[37] = 0x40; pkt[38] = 0x02; pkt[39] = 0x01;
	pkt[40] = 0;
	pkt[41] = health;
	pkt[42] = 0x87;
	float a, b, c;
	a = px + 3.4208 * sin(-angle);
	b = py + 3.4208 * cos(angle);
	angle = atan2(-(shootx - a), shooty - b);
	c = pz - 1.9226;
	float alpha = atan2(0.5,
		sqrt(pow(shootx - a, 2) + pow(shooty - b, 2)));
	printf("alpha is %f\n", alpha);
		
	
	if (angle<PI / 2 && angle>-PI / 2)
	{
		encdAngle(0, &pkt[43], &pkt[44]);
		
		if (alpha > 0)encdAngle(-PI + alpha, &pkt[45], &pkt[46]);
		else encdAngle(PI + alpha, &pkt[45], &pkt[46]);
		
		encdAngle(angle, &pkt[47], &pkt[48]);
	}
	else 
	{
		pkt[43] = 0xbf; pkt[44] = 0xff;
		
		encdAngle(alpha, &pkt[45], &pkt[46]);
		
		if(angle>0)encdAngle(PI-angle, &pkt[47], &pkt[48]);
		else encdAngle(-PI - angle, &pkt[47], &pkt[48]);
	}
	encodeCoord(a, &pkt[49], &pkt[50], &pkt[51], &pkt[52]);
	encodeCoord(b, &pkt[53], &pkt[54], &pkt[55], &pkt[56]);
	encodeCoord(c, &pkt[57], &pkt[58], &pkt[59], &pkt[60]);
	
	leftshift((unsigned char*)&pkt[40], 20);
	int iResult = send(ConnectSocket, pkt, sizeof(pkt), 0);
}
void Actor::Send93()
{
...
float decodeAngle(unsigned char a, unsigned char b)
{
	return ((a * 256 + b) - 32767 * PI) / (4096 * 4);
}
...
void Actor::Connect()
{
	...
	iResult = send(ConnectSocket, sendbuf2, sizeof(sendbuf2) - 1, 0);
	if (iResult == SOCKET_ERROR) {
				...
				|| packet[0] == 136)
			{
				...
				for (int i = 0; i < t; i++)
				{
					...
					if (message[0] == 64 || message[0] == 96
						|| message[0] == 112)
					{
						//reliable message, need ack
						if (ackSend == false)
						{
							SendACK(recvbuf);//many messages in one packet, but ack is for packet							ackSend = true;
						}
					}
					...
					/*else if (message[0] == 32 && message[10] == 0x94)
					{
						//somebody is shooting somebody
						//wait
						unsigned char shooter = message[11];
						if (message[12] == 0x50 )
						{
							//0x50 means shooting, no armour
							//0x54 means with armour
							unsigned char wepid = message[27];
							unsigned short ammo = message[28] * 256 + message[29];
							bool isfiring = message[30] == 0x40 ? true : false;
							//we are going to shift the bytes...
							leftshift(&message[33], 20);
							//34 == 0x87 or 0x83 ...
							float aimX = decodeAngle(message[35], message[36]);
							float aimY = decodeAngle(message[37], message[38]);
							float aimZ = decodeAngle(message[39], message[40]);
							
							//float posX = decodeCoord(message[41], message[42], message[43], message[44]);
							//float posY = decodeCoord(message[45], message[46], message[47], message[48]);
							//float posZ = decodeCoord(message[49], message[50], message[51], message[52]);
...
_SQUIRRELDEF(SQ_ActorShootAt) {
	...
	if (iArgCount == 5) {
		...
		actors[a].shootz = z;
		SQInteger weapon;
		sq->getinteger(v, 6, &weapon);
		actors[a].weapon = weapon;
		VCMP->SetPlayerWeapon(actors[a].clientId, weapon, 900);
		actors[a].pending4 = true;
		//actors[a].Send93();
		sq->pushinteger(v, 1);
		return 1;
...
_SQUIRRELDEF(SQ_Read) {
	...
	else printf("Usage: PlayFile(\"filename\",actorid)\n");
}

_SQUIRRELDEF(SQ_Wireshark) {
	SQInteger iArgCount = sq->gettop(v);
	if (iArgCount == 3) {
		SQInteger aid;
		sq->getinteger(v, 2, &aid);
		const SQChar* str;
		sq->getstring(v, 3, &str);
		int len = strlen(str);
		int rel; unsigned char d; unsigned char hex;
		d = str[0];
		if (d >= '0' && d <= '9')hex = d - '0';
		...
		unsigned char* packet;
		packet = new unsigned char[len+4];
		packet[0] = 132;
		...
		actors[aid].SendPacket(packet, j);
		delete[] packet;
		packet = NULL;
		return 1;
	}
...
_SQUIRRELDEF(SQ_test2)
{
	int top = sq->gettop(v);
	sq->pushroottable(v);
	sq->pushstring(v, _SC("myfunction"), -1);
	if (SQ_SUCCEEDED(sq->get(v, -2))) {
		sq->pushroottable(v);
		sq->pushstring(v, "hello", -1);
		sq->pushinteger(v, 100);
		sq->call(v, 3, 0, 0);
	}
	sq->settop(v, top);
	return 1;
...
void RegisterFuncs(HSQUIRRELVM v) {
	...
	RegisterSquirrelFunc(v, SQ_Wireshark, "wireshark", 0, 0);
	...
	RegisterSquirrelFunc(v, SQ_ActorShootAt, "actor_shoot_at", 0, 0);
}